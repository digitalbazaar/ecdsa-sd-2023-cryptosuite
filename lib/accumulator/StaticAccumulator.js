...
*/
// FIXME: nix StaticAccumulator, replacing with signatures from a single
// ephemeral key pair per VC (destroying private key once finish), ephemeral
// key binds n-quad signatures to a particular VC and allows for faster
// local signatures at issuer vs. HSM for a single signature over the ephemeral
// public key
...
export class StaticAccumulator {
  ...
  has({value, previous, witness} = {}) {
    ...
    // FIXME: witness must be signed / bound to a well-known public key
    // computation otherwise anyone can compute them and ... even then,
    // the secret value could perhaps be computed from subtracting multiple
    // witnesses + zi values, etc.
