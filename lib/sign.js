...
export async function multisign({signer, data} = {}) {
  ...
  if(nonMandatory.length > 0) {
    ...
    // FIXME: batch
    signatures = await Promise.all(nonMandatory.map(nq => localSigner(nq)));
  }

  // 2. Combine `mandatoryHash.combined`, local public key, and signatures.
  // FIXME: use CBOR
  const mhBytes = mandatoryHash?.combined || new Uint8Array(0);
  const publicKey = localKeyPair?.toMultikey() || new Uint8Array(0);
  let signaturesBytes;
  if(!signatures) {
