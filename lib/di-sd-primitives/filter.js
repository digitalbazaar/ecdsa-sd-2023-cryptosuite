...
export async function filterAndSplit({
  ...
  // 4. In parallel, canonize `filteredNQuads` to get bnode label map and
  //   split `filteredDoc` into N-Quads that match/do not match `splitFrame`.
  const labelMap = new Map();
  const [, splitResult] = await Promise.all([
    canonize(
      filteredNQuads.join(''),
      // FIXME: pass `labelMap` (bikeshed) to be populated
      {...options, inputFormat: 'application/n-quads', labelMap}),
    split({
  ...
  // 5. Return reversed bnode label map, matching and non-matching quads.
  return {labelMap: _reverseMap(labelMap), split: splitResult};
}
