...
export async function pointersToFrame({document, pointers} = {}) {
  ...
  for(const pointer of pointers) {
    ...
    if(value === undefined) {
      ...
      throw new TypeError(`JSON pointer "${pointer}" does not match document.`);
    }
    ...
    if(typeof value === 'object') {
      ...
      jsonpointer.set(frame, pointer, subframe);
      continue;
    }

    // literal selected
    const existing = jsonpointer.get(frame, pointer);
    if(existing === undefined) {
      // only override if not yet set
      jsonpointer.set(frame, pointer, value);
      // FIXME: need parent of `value` to set `@explicit` + `@requireAll` flags
      // FIXME: handle escaping?
      const parentPtr = pointer.slice(0, pointer.lastIndexOf('/'));
      const subframe = jsonpointer.get(frame, parentPtr);
      subframe['@explicit'] = true;
      subframe['@requireAll'] = true;
    }
  }
