 ...
 */
import {
  createHmac, filterAndGroup, hmacIdCanonize, pointersToFrames, select
...
async function _createDisclosureData({
  ...
  // 4. Produce mandatory frames and combined frames from pointers.
  const {selectivePointers = []} = cryptosuite.options;
  const combinedPointers = mandatoryPointers.concat(selectivePointers);
  const mandatoryFrames = pointersToFrames(
    {document, pointers: mandatoryPointers});
  const combinedFrames = pointersToFrames(
    {document, pointers: combinedPointers});
  if(!mandatoryFrames && !combinedFrames) {
  ...
  const [
    ...
    select({document, pointers: combinedPointers}),
    filterAndGroup({
      nquads,
      filterFrames: combinedFrames,
      groupFrames: mandatoryFrames,
      options
    })
  ]);
