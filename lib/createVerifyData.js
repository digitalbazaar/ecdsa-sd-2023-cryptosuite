...
export async function createVerifyData({
  ...
  */
  ...
  // 1. Parse disclosure `proof` to get parameters to verify.
  const {
  ...
  if(mandatory.length > 0) {
    ...
    const hasher = {};
    ({mandatoryHash} = await hashMandatory({mandatory, hasher}));
  }

  // 5. Build `verifyData` to pass to cryptosuite verifier.
  // FIXME: compute and include `proofHash`
